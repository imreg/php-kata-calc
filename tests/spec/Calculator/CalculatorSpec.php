...
{
    function let(BitwiseKeyboard $keyboard)
    {
        $keyboard->keys()->willReturn('0-9.+\-\*\/&\|\^\~\<\>');
        $this->beConstructedWith($keyboard);
    }

    function it_return_zero_for_empty_array(BitwiseKeyboard $keyboard)
    {
    ...
    function it_return_two_if_one_plus_one_were_submitted(BitwiseKeyboard $keyboard)
    {
    ...
    function it_return_three_if_five_minus_two_were_submitted(BitwiseKeyboard $keyboard)
    {
        $keyboard->submit()->willReturn([5, '-', 2]);
        $this->result()->shouldReturn(3);
    }

    function it_return_nine_if_three_times_three_were_submitted(BitwiseKeyboard $keyboard)
    {
        $keyboard->submit()->willReturn([3, '*', 3]);
        $this->result()->shouldReturn(9);
    }

    function it_return_five_if_ten_devided_by_two_were_submitted(BitwiseKeyboard $keyboard)
    {
        $keyboard->submit()->willReturn([10, '/', 2]);
        $this->result()->shouldReturn(5);
    }

    function it_return_seven_if_two_or_five_were_submitted(BitwiseKeyboard $keyboard)
    {
        $keyboard->submit()->willReturn([2, '|', 5]);
        $this->result()->shouldReturn(7);
    }

    function it_return_four_if_one_xor_five_were_submitted(BitwiseKeyboard $keyboard)
    {
        $keyboard->submit()->willReturn([1, '^', 5]);
        $this->result()->shouldReturn(4);
    }

    function it_return_zero_if_two_xor_five_were_submitted(BitwiseKeyboard $keyboard)
    {
        $keyboard->submit()->willReturn([2, '&', 5]);
        $this->result()->shouldReturn(0);
    }
